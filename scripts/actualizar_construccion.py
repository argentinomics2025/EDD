...
# --- CABECERAS PARA EVITAR BLOQUEOS DE SERVIDORES PÚBLICOS ---
DEFAULT_HEADERS = {
...
def run():
    ...
    try:
        ...
        r_act = requests.get(url_isac, headers=DEFAULT_HEADERS, timeout=20)
        ...
        for i in range(1, len(filas)):
            ...
            # Validación de seguridad: evitar división por cero si la API manda datos rotos
            if val_actual is not None and val_previo is not None and val_previo > 0:
        ...
        print(f"⚠️ Error de red consultando ISAC: {e}")
    ...
    # ==========================================================
    try:
        print("\n👉 Buscando Consumo de Insumos (Materiales)...")
        ...
        r_csv = requests.get(url_csv, headers=DEFAULT_HEADERS, timeout=20)
            ...
            for mat_supabase, col_csv in mapeo_columnas.items():
                ...
                if val_actual_str.strip():
                    try:
                        ...
                        # Si el CSV trae texto o guiones en lugar de números, lo ignoramos
                        pass
        ...
        print(f"⚠️ Error al descargar CSV de Insumos: {e}")
    except Exception as e:
        print(f"❌ Error General en Insumos: {e}")

    print("\n🚀 ¡Robot de Construcción completado con éxito!")
